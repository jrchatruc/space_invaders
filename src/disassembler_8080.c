...
int disassemble_8080(char filepath[])
{
    ...
    if (fp != NULL)
    {
        if (fseek(fp, 0L, SEEK_END) == 0)
        {
            ...
            buffer = malloc(sizeof(unsigned char) * (bufsize + 1));
            ...
            size_t newLen = fread(buffer, sizeof(unsigned char), bufsize, fp);
            if (ferror(fp) != 0)
            {
            ...
            while (pc < newLen)
            {
                opbytes = disassemble_8080_op(buffer, pc);
                pc += opbytes;
            }
            free(buffer);
        }
...
int disassemble_8080_op(unsigned char *codebuffer, int pc)
{
    ...
    case 0xcc:
        ...
        break;
    case 0xcd:
        printf("CALL    $0x%02x%02x", code[2], code[2]);
        opbytes = 3;
        break;
    case 0xce:
